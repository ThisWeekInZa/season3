...
export class AdminTenantUsersListComponent implements OnInit {
  ...
  listConfig: ItemListConfig = {
    ...
    dataService: {
      ...
      deleteItem: (params, item) =>
        this.tenantAccessService.removeUserFromTenant(this.tenantId, item.id),
    },
  };
      ...
      selectItems: async (items) => {
        const promises = items.map(async (user) => {
          return firstValueFrom(
            this.tenantAccessService.addUserToTenant(this.tenantId, user.id)
          );
        });
  ...
  constructor(
    ...
    private tenantAccessService: TenantService,
