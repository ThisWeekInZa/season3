...
export class MockAuthService {
  ...
  private mockTokenExpiry = new Date(Date.now() + 3600 * 1000).toString(); // 1 hour from now
  private mockRefreshTokenExpiry = new Date(
    Date.now() + 86400 * 1000
  ...
  private setupMockAuth(): void {
    ...
    localStorage.setItem(
      this.TENANTS_KEY,
      JSON.stringify(this.mockUser.tenants)
    );
  ...
  private hasValidToken(): boolean {
    return true; // Always return true in mock service
  }
  ...
  logout() {
    // In mock service, we clear storage but immediately setup mock auth again
    localStorage.clear();
    this.setupMockAuth();
  }
  ...
  getUserInitials(): string {
    return `${this.mockUser.firstName.charAt(0)}${this.mockUser.lastName.charAt(
      0
    )}`.toUpperCase();
  }
  ...
  handleUnauthorized() {
    ...
    // No actual logout in mock service
  }

  isTokenExpired(): boolean {
    return false; // Never expired in mock service
  }
  ...
  async redirectToMicrosoftLogin() {
    ...
    // Instead of redirecting, simulate successful login
